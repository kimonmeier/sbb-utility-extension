...
async function synchronizeTouren(employeeId: string, employeeIndentification: string, api_token: string) {
  ...
  const uniqueTourItemsByDay = buildUniqueTourItemsByDay(
    flattenTourItems(tourenData),
  );
...
async function synchronizeZeitkonten(employeeId: string, token: string) {
  ...
  try {
    ...
    throw new Error(
      toUserFacingSbbError(
        error,
        "Failed to fetch Zeitkonten data from SBB API.",
      ),
      {
        cause: error,
      },
  ...
  const snapshots: SBBUtilityZeitkontoSnapshot[] = selectedEntries.map(
    (entry) => ({
...
async function processTourenData(
  ...
  return Promise.all(
    tourItems.map(async (item) => {
      ...
      const tourDetail = await fetchTourDetailSafely(
        token,
        item.mitarbeiterTourId,
      );
...
async function fetchTourenYearDataOrThrow(
  token: string,
): Promise<SopreMonthsRequest> {
  ...
  try {
    ...
    throw new Error(
      toUserFacingSbbError(error, "Failed to fetch touren data from SBB API."),
      {
        cause: error,
      },
    );
...
function chooseTourItemForDay(items: TourItem[]): TourItem {
  ...
  if (
    ...
    items.find(
      (x) => TOUR_TYPE_BY_CODE[x.abkuerzung!] === SopreTourType.FERIEN,
    ) &&
    (items.find(
      (x) => TOUR_TYPE_BY_CODE[x.abkuerzung!] == SopreTourType.RUHETAGE,
    ) ||
      items.find(
        (x) =>
          TOUR_TYPE_BY_CODE[x.abkuerzung!] === SopreTourType.KOMPENSATIONSTAG,
      ))
  ) {
    return items.find(
      (x) => TOUR_TYPE_BY_CODE[x.abkuerzung!] === SopreTourType.FERIEN,
...
async function upsertTourenForUser(
  ...
  const toInsert = processedTouren.filter(
    (row) => !existingDays.has(row.datum),
  );
...
async function fetchTourDetailSafely(
  token: string,
  mitarbeiterTourId?: number | null,
) {
...
function applyTourDetailFields(
  ...
  if (tourDetail?.tourdetailDTO) {
    tour.schichtdauer = parseDurationToMinutes(
      tourDetail.tourdetailDTO.schichtdauer,
    );
    tour.arbeitszeit = parseDurationToMinutes(
      tourDetail.tourdetailDTO.arbeitszeit,
    );
    tour.bezahlteZeit = parseDurationToMinutes(
      tourDetail.tourdetailDTO.bezahlteZeit,
    );
    tour.bezahltePause = parseDurationToMinutes(
      tourDetail.tourdetailDTO.bezahltePause,
    );
  }
...
function flattenTourItems(tourenData: SopreMonthsRequest) {
  return tourenData.flatMap((month) =>
    month.weekDTOs.flatMap((week) =>
      week.dayDTOs.flatMap((day) => day.dayItemDTOs || []),
    ),
  );
}
