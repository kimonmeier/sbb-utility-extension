...
const BASE_ZEITKONTEN_API_ENDPOINT =
  "https://sopreweb-zeitkonten-api.app.sbb.ch";
...
async function readResponseBodySafe(
  response: Response,
  ...
  for (let attempt = 1; attempt <= MAX_RETRY_ATTEMPTS; attempt++) {
    ...
    if (!response.ok) {
      const shouldRetryStatus =
        isRetryableStatus(response.status) && attempt < MAX_RETRY_ATTEMPTS;
...
export function toUserFacingSbbError(
  error: unknown,
